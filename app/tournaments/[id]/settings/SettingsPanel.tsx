  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in lg:py-4">
      ...
      <div className="relative rounded-2xl p-5 overflow-hidden" style={{ background: 'linear-gradient(135deg, #161616 0%, #0A0A0A 100%)', border: '1px solid #242424' }}>
        <Link href={`/tournaments/${tournament.id}`} className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-[#888888] hover:text-[#F0F0F0] transition-colors mb-2">
          ← {tournament.name}
        </Link>
      ...
      {/* Courts */}
      <Link
        href={`/tournaments/${tournament.id}/tv-admin`}
      ...
      <ResultadoQRCard tournamentId={tournament.id} />
...
// ── Self-service result QR — one link for the whole tournament ────
// Meant for a single shared tablet sitting near the courts: the organizer
// scans once, sees a card per court for whatever match is happening right
// now, taps the one that just finished, and enters the score.

function ResultadoQRCard({ tournamentId }: { tournamentId: string }) {
  const [dataUrl, setDataUrl] = useState<string | null>(null)
  const [copied, setCopied]   = useState(false)
  const [url, setUrl]         = useState('')

  useEffect(() => {
    const link = `${window.location.origin}/t/${tournamentId}/lancar-resultado`
    setUrl(link)
    let cancelled = false
  ...
  return (
    <div className="bg-[#161616] border border-[#242424] rounded-2xl overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[#242424] flex items-center gap-2">
        ...
        <p className="text-xs font-black text-[#888888] uppercase tracking-widest">Autoatendimento de resultado</p>
      </div>
      <div className="px-5 py-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          <p className="text-xs font-bold text-[#888888] leading-snug">
            Um QR único para o torneio inteiro — deixe num tablet perto das quadras. Quem escaneia vê as partidas
            acontecendo agora e lança o resultado direto.
          </p>
          {url && (
